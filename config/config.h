...
#define CNF_TIMER_PERIOD	1	/* System timer period (1ms tick, matches FreeRTOS configTICK_RATE_HZ=1000) */

/* Maximum number of kernel objects.
 * WiFi+lwIP+libc create ~15 tasks, ~50 sems, ~40 mutexes, ~20 queues (mbf).
 * Each MTXCB ~32B, SEMCB ~28B, MBFCB ~32B — bumping is cheap in BSS. */
#define CNF_MAX_TSKID		64	/* Task (48→64: ESP-MESH adds ~6 tasks) */
#define CNF_MAX_SEMID		128	/* Semaphore (64→128: shim uses 1 sem/task + 2/queue;
					   ESP-MESH exhausted 64.  Cheap: ID table grows in
					   BSS, control blocks alloc on demand) */
#define CNF_MAX_FLGID		32	/* Event flag */
#define CNF_MAX_MBXID		8	/* Mailbox*/
#define CNF_MAX_MTXID		64	/* Mutex (libc ~10 + WiFi ~20 + lwIP ~10 + misc ~10) */
#define CNF_MAX_MBFID		32	/* Message buffer (FreeRTOS queues → mbf) */
#define CNF_MAX_MPLID		4	/* Memory pool */
#define CNF_MAX_MPFID		8	/* Fixed size memory pool */
#define CNF_MAX_CYCID		16	/* Cyclic handler (FreeRTOS periodic timers) */
#define CNF_MAX_ALMID		32	/* Alarm handler (FreeRTOS one-shot timers) */
...
#define CNF_MAX_PORID		(2)	/* Maximum number of Rendezvous */
